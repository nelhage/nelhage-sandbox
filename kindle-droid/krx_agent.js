  ...
  "agent_src.js"() {
    ...
    function findSDK() {
      ...
      for (const cn of SDK_CLASSES) {
        let found = null;
        try {
          frida_java_bridge_default.choose(cn, {
            onMatch(inst) {
              found = inst;
              return "stop";
            },
    ...
    function currentActivity() {
      ...
      const map = at.mActivities.value;
      const vals = map.values().toArray();
      for (let i = 0; i < vals.length; i++) {
        const rec = frida_java_bridge_default.cast(vals[i], frida_java_bridge_default.use("android.app.ActivityThread$ActivityClientRecord"));
        let paused = true;
        try {
          paused = rec.paused.value;
        } catch (e) {
        }
        if (!paused) return rec.activity.value;
      }
      if (vals.length) {
        const rec = frida_java_bridge_default.cast(vals[0], frida_java_bridge_default.use("android.app.ActivityThread$ActivityClientRecord"));
    ...
    rpc.exports = {
      ...
      info(asin) {
        return new Promise((resolve) => {
          frida_java_bridge_default.perform(() => {
            const sdk = findSDK();
            if (!sdk) {
              resolve({ ok: false, err: "no SDK instance found" });
              return;
            }
            let book = null, binfo = {};
            try {
              ...
              if (book) {
                ...
                try {
                  binfo.state = "" + book.getContentState();
                } catch (e) {
                  binfo.state = "n/a";
                }
              }
      ...
      download(asin) {
        return new Promise((resolve) => {
          frida_java_bridge_default.perform(() => onMain(() => {
            const sdk = findSDK();
      ...
      removeDownload(asin) {
        return new Promise((resolve) => {
          frida_java_bridge_default.perform(() => onMain(() => {
            const sdk = findSDK();
            const LM = frida_java_bridge_default.use("com.amazon.kindle.krx.library.LibraryManager");
            const lm = frida_java_bridge_default.cast(sdk.getLibraryManager(), LM);
