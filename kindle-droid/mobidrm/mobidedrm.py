...
# Vendored subset for kindle-droid: only the PC1 cipher is needed; the CLI
# helpers (SafeUnbuffered/unicode_argv) and PID-based getUnencryptedBook path
# were dropped.  We recover `found_key` from device memory instead of from a
# PID (processBookWithKey), since this app version's account secrets are
# AES-GCM-wrapped and out of reach of DeDRM's androidkindlekey.
from .pc1 import PC1 as _PC1


class Pukall_Cipher(object):
    def PC1(self, key, src, decryption=True):
        return _PC1(key, src, decryption)
...
class MobiBook:
    ...
    # --- kindle-droid additions --------------------------------------------
    # Decrypt a crypto-type-2 book with a `found_key` recovered from device
    # memory, skipping the PID/DRM-cookie derivation entirely.  Mirrors the
    # crypto_type==2 tail of processBook() (record decrypt + DRM-record wipe),
    # minus parseDRM.  Returns nothing; result is in self.mobi_data.
    def processBookWithKey(self, found_key):
        crypto_type, = struct.unpack('>H', self.sect[0xC:0xC+2])
        self.crypto_type = crypto_type
        if crypto_type == 0:
            self.print_replica = (self.loadSection(1)[0:4] == b'%MOP')
            ...
            return
        if crypto_type != 2:
            raise DrmException("processBookWithKey only supports crypto type 2, "
                               "not {0:d}".format(crypto_type))
        # wipe the (now-unused) DRM cookie block + pointers so the output is
        # genuinely DRM-free, exactly as processBook() does.
        drm_ptr, drm_count, drm_size, drm_flags = struct.unpack(
            '>LLLL', self.sect[0xA8:0xA8+16])
        if drm_size:
            self.patchSection(0, b'\0' * drm_size, drm_ptr)
        self.patchSection(0, b'\xff' * 4 + b'\0' * 12, 0xA8)
        self.patchSection(0, b'\0' * 2, 0xC)   # clear crypto type

        mobidataList = [self.data_file[:self.sections[1][0]]]
        for i in range(1, self.records + 1):
            data = self.loadSection(i)
            extra_size = getSizeOfTrailingDataEntries(
                data, len(data), self.extra_data_flags)
            mobidataList.append(PC1(found_key, data[0:len(data) - extra_size]))
            if extra_size > 0:
                mobidataList.append(data[-extra_size:])
        if self.num_sections > self.records + 1:
            mobidataList.append(self.data_file[self.sections[self.records+1][0]:])
...
def decrypt_with_key(infile, found_key, outfile):
    """DRM-strip a crypto-type-2 Mobipocket/KF8 file with a known 16-byte key."""
    book = MobiBook(infile)
    book.processBookWithKey(found_key)
    with open(outfile, 'wb') as f:
        f.write(book.mobi_data)
    return book
