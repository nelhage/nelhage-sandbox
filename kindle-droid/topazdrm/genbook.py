...
def generateBook(bookDir, raw, fixedimage):
    ...
    # getPageDim returns bytes (b'0' for a zero-dimension page, e.g. when the
    # chosen "first text page" is actually a cover/front-matter page); the
    # original str comparisons here never matched the bytes, letting a 0 through
    # to int(ph)*1.0 == 0.0 and a divide-by-zero in stylexml2css.  Coerce and
    # fall back to letter-size defaults for any missing/zero/invalid dimension.
    def _dim(v, default):
        if isinstance(v, bytes): v = v.decode('latin1', 'ignore')
        try:
            return v if float(v) > 0 else default
