...
# kgenpids (offline PID generation) is only used by cli_main's PID path, which we
# never take — we set the 8-byte bookKey directly from the heap.  Guard the import
# so this module loads without kgenpids/pycrypto present.
try:
    if inCalibre:
