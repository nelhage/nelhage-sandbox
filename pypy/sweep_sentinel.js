"use strict";
// Sweep sentinel_sum throughput across p(sentinel). Mirrors sweep_sentinel.py.
//     node sweep_sentinel.js [fractions...]
...
function buildData(n, frac) {
  ...
  for (let i = 0; i < n; i++) if (rng() < frac) data[i] = SENTINEL;
  return data;
}

function bestTime(data) {
  sentinelSum(data); // warm-up for this dataset
  let best = Infinity;
...
console.log(`  ${"p(sent)".padStart(7)} ${"best (s)".padStart(10)} ${"M elem/s".padStart(10)}`);
